...
function sum(object) {
  let arr = [];
  if(!Object.getOwnPropertyNames(object)) return 0;
  Object.values(object).forEach(value => { !isNaN(value) && arr.push(value) });
...
function addAll(arr) {
  const target = {};
  arr.forEach(el => Object.assign(target, el));
  return target;
}
...
function find(obj, matcherFunc) {
  ...
  for(i = 0; i < arr.length; i++) {
    if (matcherFunc(arr[i])) return arr[i];
  }
  return null;
}
...
function every(obj, func) {
  const arr = Object.values(obj);
  return arr.every(func) ? true : false;
}
...
/*
  ...
  and a tester function. If at least 1 of the values in the object pass the tester function, true is returned.
